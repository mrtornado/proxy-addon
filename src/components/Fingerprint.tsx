...
export default function Fingerprint({ member_key }: any) {
  ...
  const fingerprint = (): Promise<{
    ...
    function isFirefox(): boolean {
      ...
      return (
        (document.documentElement as any).style.MozAppearance !== undefined
      );
    }
    ...
    return new Promise(function (resolve, reject): void {
      const fingerprints = {
        ...
        evalToString: (): Promise<[number, number]> => {
          // Use an example function instead of `eval`
          function exampleFunction() {}
          return Promise.resolve([0, exampleFunction.toString().length]);
        },
        ...
        reducedMotion: (): Promise<[number, any]> => {
          return new Promise((resolve): void => {
            function prm(x: string): boolean {
              return Boolean(
                matchMedia("(prefers-reduced-motion: " + x + ")").matches
              );
            }
        ...
        contrast: (): Promise<[number, any]> => {
          return new Promise((resolve): void => {
            function pc(x: string): boolean {
              return Boolean(
                matchMedia("(prefers-contrast: " + x + ")").matches
              );
            }
            resolve([
              0,
        ...
        invertedColors: (): Promise<[number, any]> => {
          return new Promise((resolve): void => {
            function ic(x: string): boolean {
              return Boolean(
                matchMedia("(inverted-colors: " + x + ")").matches
              );
            }
        ...
        browserObjects: (): Promise<[number, any]> => {
          return new Promise((resolve): void => {
            ...
            for (let i = 0; i < objects.length; i++) {
              if (typeof window[objects[i] as any] === "object")
        ...
        screenResolution: (): Promise<[number, any]> => {
          return new Promise((resolve): void => {
            ...
            resolve([
              0,
              [Number(screen.width), Number(screen.height)]
                .sort()
                .reverse()
                .join("x"),
            ]);
        ...
        canvasAPI: (): Promise<[number, any]> => {
          return new Promise((resolve): void => {
            ...
            const canvas_geometry_fp = murmurhash3_32_gc(
              canvas.toDataURL(),
              420
            );
            const combined_winding = canvas_text(ctx);
            const canvas_combined_fp = murmurhash3_32_gc(
              canvas.toDataURL(),
              420
            );
          ...
          return new Promise((resolve): void => {
            ...
            function populateVoiceList(): void {
              ...
              if (output.length > 0 || tripped) {
                resolve(
                  output.length > 0
                    ? [0, murmurhash3_32_gc(JSON.stringify(output), 420)]
                    : [-3, null]
                );
              }
              tripped = true;
        ...
        webglInfo: (): Promise<[number, any]> => {
          return new Promise((resolve): void => {
            ...
            const debugExtension = context.getExtension(
            ...
            output.shaderVersion = context.getParameter(
            ...
            for (let i = 0; i < parameterNames.length; i++) {
              output.parameters.push(
                parameterNames[i] +
                  "=" +
                  context.getParameter(context[parameterNames[i]])
              );
            }
            ...
            for (let i = 0; i < shaderTypes.length; i++) {
              ...
              for (let j = 0; j < precisionTypes.length; j++) {
                output.shaderPrecision.push(
                  getShaderPrecision(shaderType, precisionTypes[j])
                );
              }
            }
            ...
            for (let i = 0; i < vendorPrefixes.length; i++) {
              ...
              for (let extension in extensionList) {
                if (extensionList.hasOwnProperty(extension)) {
                  ...
                  const supported = context.getExtension(
                    vendorPrefix + extension
                  );
                  if (supported) {
                    for (let j = 0; j < extensionParameters.length; j++) {
                      const extensionParameter = extensionParameters[j];
                      const extensionParameterValue =
                        supported[extensionParameter];
            ...
            output.attributes = murmurhash3_32_gc(
              JSON.stringify(output.attributes),
              420
            );
            output.parameters = murmurhash3_32_gc(
      ...
      Promise.all(promises)
        .then((k): void => {
          ...
          const persistentFp = murmurhash3_32_gc(
            JSON.stringify(persistentFpComponetents),
            420
          );
          const output = {
  ...
  fingerprint()
    .then((result) => {
      ...
      fetch(`${apiUrl}/api/fingerprint`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
        body: JSON.stringify({
          member_key: member_key, // Replace with the actual member_key
          fingerprint: result.fingerprints.uniqueFp,
        }),
      })
        ...
        .then((data) => {
          ...
          if (data.existingFingerprint.trial) {
            // This user already has a trial
            if (location.pathname.includes("trial")) {
              ...
              return alert(
                `You already used your trial on the account that's using the next email address ${data.existingFingerprint.email_address} . Please don't abuse our kindness.`
              );
            }
          }
        })
        .catch((error) => {
          console.error(
            "Error sending fingerprint data to the backend:",
            error
          );
        });
    })
